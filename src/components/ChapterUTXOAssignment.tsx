            ...
            >
              {clientSubmissionIsApproved(submissionStatus, phaseHint)
                ...
                : clientAwaitingInstructorReview(submissionStatus, phaseHint)
                  ...
                  : clientNeedsResubmit(submissionStatus, phaseHint)
                    ? 'Try again — selection was not accepted'
                    : explanation?.title || 'Assignment Submitted'}
            </p>
            {clientAwaitingInstructorReview(submissionStatus, phaseHint) && (
              <p className="text-sm text-zinc-300">
                Your instructor will approve or return this. You cannot change your selection until then.
              </p>
            )}
            {clientAutoGradedCanPractice(submissionStatus, phaseHint) && (
              <p className="text-xs text-zinc-500 mt-2">Practice again below — your sats credit stays once earned.</p>
            )}
